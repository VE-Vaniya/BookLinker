...
function ViewExchangerTransection() {
  ...
  const processTransactionData = () => {
    // Process both sold items and exchanged items
    const relevantItems = data.filter((transaction) =>
      ["sold", "exchanged_out", "exchanged_in"].includes(transaction.status)
    );

    return relevantItems.map((transaction) => {
      // Custom props based on transaction type
      let buttonColor = "red";
      let buttonContent = transaction.status;
      let subtitle = "";

      // Customize the display based on transaction type
      if (transaction.status === "exchanged_out") {
        buttonColor = "amber";
        buttonContent = "Exchanged";
        subtitle = `Exchanged for: ${transaction.exchangeBookName}`;
      } else if (transaction.status === "exchanged_in") {
        buttonColor = "green";
        buttonContent = "Received";
        subtitle = `Received in exchange for: ${transaction.exchangeBookName}`;
      ...
      return (
        <div key={transaction.id} className="m-2.5">
          <Card
            data={{
              ...transaction,
              subtitle: subtitle, // Add subtitle for exchange information
            }}
...
export default ViewExchangerTransection;
