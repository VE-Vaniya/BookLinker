...
function ViewLendTransection() {
  ...
  const processTransactionData = () => {
    const lendedItems = data.filter(
      (transaction) => transaction.status === "lended"
    );
  
    return lendedItems.map((transaction) => (
      <div key={transaction._id} className="m-2.5">
        <Card
          data={transaction}
          buttonColor={"blue"} // changed from green to blue for lending
          buttonContent={"Lended"}
        />
      </div>
    ));
  };
      ...
      <main className="flex-1 p-10 text-white">
        <div className="flex flex-col items-center mb-10 lg:flex-row lg:justify-between lg:items-center">
          <div className="text-center lg:text-left mb-4 lg:mb-0">
            <h1 className="text-2xl font-semibold">Lending History</h1>
            <p className="text-sm text-white/70">View your Lending items</p>
          </div>
...
export default ViewLendTransection;
