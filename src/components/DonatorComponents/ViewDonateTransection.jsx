...
function ViewDonateTransection() {
  ...
  const processTransactionData = () => {
    const donatedItems = data.filter(
      (transaction) =>
        transaction.status === "donated" || transaction.status === "sold"
    );

    return donatedItems.map((transaction) => (
      <div key={transaction._id} className="m-2.5">
        <Card
          data={transaction}
          buttonColor={"green"}
          buttonContent={transaction.status === "donated" ? "Donated" : "Sold"}
        />
      </div>
    ));
  };
      ...
      <main className="flex-1 p-10 text-white">
        <div className="flex flex-col items-center mb-10 lg:flex-row lg:justify-between lg:items-center">
          <div className="text-center lg:text-left mb-4 lg:mb-0">
            ...
            <p className="text-sm text-white/70">View your donated items</p>
          </div>
...
export default ViewDonateTransection;
